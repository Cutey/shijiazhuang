  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="typeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="文件后缀" align="center" prop="fileSuffix" >
        <template slot-scope="scope">
           <span v-if="scope.row.resourcesName.substring(scope.row.resourcesName.length-4)=='json'">.json</span>
          <span v-else-if="scope.row.fileSuffix =='A'">.png</span>
      ...
      <el-table-column label="文件日期" align="center" prop="resourcesDate" width="200"/>
      <el-table-column label="创建日期" align="center" prop="createDate" width="200"/>
       <el-table-column label="文件预览" align="center" prop="status" :formatter="statusFormat" >
            <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
            icon="el-icon-picture"
            @click="filelook(scope.row)"
          >文件预览</el-button>
...
<script>
import { getProducer, insertProducer,deleteProducer,updateProducer,getDResourceTypeKV} from "@/api/zygl/cpwj";
import Pdfmoud from '../../peopleshenhe/reportaudit/commond/pdfmoud'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getProducer(this.queryParams).then(response => {
    ...
    filelook(row){
        ...
        this.pdfurl =''
          let url= process.env.VUE_APP_BASE_API + process.env.VUE_APP_REPORT_PATH + row.resourcesUrl
           ...
           if(row.resourcesName.substring(row.resourcesName.length-4)=='json'){

           }else{
              if(row.fileSuffix =='E'){//如果文件类型是pdf就预览
                this.downobj = row
                this.isshowPdf = true
                this.pdfurl = url
            }else if(row.fileSuffix =='D'){//文件类型是doc
              
            }else if(row.fileSuffix =='A' || row.fileSuffix =='B' || row.fileSuffix =='C'){//文件类型是doc
               this.lb.push(url)
               this.picAmplify()
            }else{
